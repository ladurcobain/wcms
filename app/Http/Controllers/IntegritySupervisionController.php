...
class IntegritySupervisionController extends Controller
{
    private $title = "Zona Integritas";
    private $subtitle = "Penguatan Pengawasan";
    private $acc_menu = 67;
        ...
        if($slug != "") {
            ...
            if($access != "[]") {
                if (in_array($this->acc_menu, $access)) {
                    $uri = Curl::endpoint();
                    $url = $uri .'/'.'v1/get-integrity-supervision';
                    ...
                    if($res->status == true) {
                        ...
                        return view('integrity.supervision', $data, compact('info'));
                    }
                    else {
                        return redirect('/');  
                    }  
