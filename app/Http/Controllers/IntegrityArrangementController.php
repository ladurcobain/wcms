...
class IntegrityArrangementController extends Controller
{
    private $title = "Zona Integritas";
    private $subtitle = "Penataan Tata Laksana";
    private $acc_menu = 63;
        ...
        if($slug != "") {
            ...
            if($access != "[]") {
                if (in_array($this->acc_menu, $access)) {
                    $uri = Curl::endpoint();
                    $url = $uri .'/'.'v1/get-integrity-arrangement';
                    ...
                    if($res->status == true) {
                        ...
                        return view('integrity.arrangement', $data, compact('info'));
                    }
                    else {
                        return redirect('/');  
                    }  
