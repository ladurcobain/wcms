...
class IntegrityMechanismController extends Controller
{
    private $title = "Zona Integritas";
    private $subtitle = "Manajemen Perubahan";
    private $acc_menu = 62;
        ...
        if($slug != "") {
            ...
            if($access != "[]") {
                if (in_array($this->acc_menu, $access)) {
                    $uri = Curl::endpoint();
                    $url = $uri .'/'.'v1/get-integrity-mechanism';
                    ...
                    if($res->status == true) {
                        ...
                        return view('integrity.mechanism', $data, compact('info'));
                    }
                    else {
                        return redirect('/');  
                    } 
