...
class InformationUnitController extends Controller
{
    ...
    private $subtitle = "Bidang Organisasi";
    private $acc_menu = 31;
        ...
        if($slug != "") {
            ...
            if($access != "[]") {
                if (in_array($this->acc_menu, $access)) {
                    ...
                    $uri = Curl::endpoint();
                    $url = $uri .'/'.'v1/get-information-unit';
                    $param = array(
                    ...
                    if($res->status == true) {
                        ...
                        return view('information.unit.index', $data, compact('info', 'results'));
                    }
                    else {
                        return redirect('/');  
                    }
    ...
    public function read($id="")
    {
        ...
        if($access != "[]") {
            if (in_array($this->acc_menu, $access)) {
                if($id != "") {
                    $uri = Curl::endpoint();
                    $url = $uri .'/'.'v1/read-information-unit/'. $slug .'/@'. $id .'&more';
                ...
                if($res->status == true) {
                    ...
                    return view('information.unit.read', $data, compact('info'));
                }
                else {
                    return redirect('/');  
                }
