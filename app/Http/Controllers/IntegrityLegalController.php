...
class IntegrityLegalController extends Controller
{
    private $title = "Zona Integritas";
    private $subtitle = "Dasar Hukum";
    private $acc_menu = 61;
        ...
        if($slug != "") {
            ...
            if($access != "[]") {
                if (in_array($this->acc_menu, $access)) {
                    $uri = Curl::endpoint();
                    $url = $uri .'/'.'v1/get-integrity-legal';
                    ...
                    if($res->status == true) {
                        ...
                        return view('integrity.legal', $data, compact('info'));
                    }
                    else {
                        return redirect('/');  
                    }  
