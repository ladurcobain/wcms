...
class IntegrityProfessionalismController extends Controller
{
    private $title = "Zona Integritas";
    private $subtitle = "Penataan Manajemen SDM";
    private $acc_menu = 65;
        ...
        if($slug != "") {
            ...
            if($access != "[]") {
                if (in_array($this->acc_menu, $access)) {
                    $uri = Curl::endpoint();
                    $url = $uri .'/'.'v1/get-integrity-professionalism';
                    $param = array(
                        'slug' => $slug,
                    ...
                    if($res->status == true) {
                        ...
                        return view('integrity.professionalism', $data, compact('info'));
                    }
                    else {
                        return redirect('/');  
                    }  
